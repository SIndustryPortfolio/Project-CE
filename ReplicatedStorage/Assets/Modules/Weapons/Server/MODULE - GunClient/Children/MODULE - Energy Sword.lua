...
-- Functions
-- MECHANICS
local function UpdateEnergy(GunModel)
	-- Functions
	-- INIT
	if GunModel:GetAttributes()["Energy"] > 0 then
		return nil
	end
...
local function Initialise(GunClientModule, GunModel)
	-- Functions
	-- DIRECT
	local Connection1 = GunModel:GetAttributeChangedSignal("Energy"):Connect(function()
		return UpdateEnergy(GunModel)	
	end)
