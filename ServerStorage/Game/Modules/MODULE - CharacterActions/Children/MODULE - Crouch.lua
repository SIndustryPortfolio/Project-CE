...
local function Crouch(CharacterActionsModule, Player)
	...
	if Humanoid:GetAttributes()["BaseSpeed"] then
		DefaultWalkSpeed = Humanoid:GetAttributes()["BaseSpeed"]
	end
	
	if table.find(CollectionService:GetTags(Character), "Custom Power Up") then
		Humanoid.WalkSpeed = PowerUpsInfoModule:GetPowerUpInfo("Custom Power Up")["WalkSpeed"] * CharacterInfoModule:GetCharacterInfo("CrouchSpeedMultiplier")
	else
		Humanoid.WalkSpeed = DefaultWalkSpeed * CharacterInfoModule:GetCharacterInfo("CrouchSpeedMultiplier")
	end
	
	--Humanoid.WalkSpeed = Humanoid.WalkSpeed * CharacterInfoModule:GetCharacterInfo("CrouchSpeedMultiplier")
	Humanoid:SetAttribute("Crouch", true)
end
...
-- DIRECT
function CrouchModule.Initialise(NilParam, ...)
	return Crouch(...)
end

function CrouchModule.End()
	return End()
end
